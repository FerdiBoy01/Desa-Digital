...
import { 
    ...
    FaCamera, FaCheck, FaTimes, FaHandHoldingHeart, FaUserCheck, 
    FaLightbulb, FaRobot, FaBullhorn, FaUserSecret, FaExclamationTriangle,
    FaCheckCircle 
} from "react-icons/fa";
...
const SurveyDetail = () => {
  ...
  const handleVerification = async (status) => {
    ...
    if(status === "Ditolak") {
        ...
        // Cek dulu apakah ada laporan warga sebelum menyetujui
        if(biodata.comments && biodata.comments.length > 0) {
             const ignoreWarning = window.confirm("PERHATIAN: Ada laporan warga untuk data ini. Apakah Anda yakin tetap ingin MENYETUJUI?");
    ...
    <LayoutAdmin>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {/* --- FITUR BARU: PANEL LAPORAN WARGA (SOCIAL CONTROL) --- */}
            {biodata.comments && biodata.comments.length > 0 ? (
                <div className="bg-red-50 border-l-4 border-red-500 rounded-lg p-6 shadow-sm">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-red-800">Laporan Masyarakat Diterima!</h3>
                            <p className="text-sm text-red-600">
                                Warga memberikan {biodata.comments.length} tanggapan untuk penerima ini. 
                                Harap verifikasi ulang sebelum menyetujui.
                            </p>
                        </div>
                    </div>

                    <div className="space-y-3 pl-2 md:pl-14 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                        {biodata.comments.map((comment, index) => (
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-green-800 text-sm">Tidak Ada Laporan Negatif</h3>
                        <p className="text-xs text-green-600">Belum ada tanggapan publik yang masuk untuk data ini.</p>
                    </div>
                </div>
            )}
